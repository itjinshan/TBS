...
- **Time-budget-aware itinerary generation, including mode of transportation.** `Services/itineraryPlanner.js` currently allocates a fixed `SPOTS_PER_DAY` (3) rather than modeling how much a real day actually holds. A real fix needs several pieces working together: (1) use each spot's already-collected `AverageTimeSpent` to size a day by total time, not spot count; (2) calculate real travel time between consecutive spots, not just minimize geographic distance (today's nearest-neighbor ordering reduces zigzagging but doesn't budget a clock against it); (3) ask the traveler's **mode of transportation** (public transit, taxi/rideshare, or driving a rented/owned car) during intake — the same distance implies very different travel times depending on mode, so this needs to be a new field in the trip-intake flow (`APIs/trip.js`'s `STAGES`/`nluExtraction.js`) feeding into the travel-time calculation in (2); (4) reserve time for meals and rest, not just back-to-back spots. Surfaced as a real product gap while documenting the itinerary-generation pipeline in `README.md` — see that file's "Known limitations" for the user-facing framing.
...
- **Auth flow gaps found during a manual review of `APIs/auth.js`/`Config/jwtgenerator.js`/`Config/passport.js` (two concrete bugs from this review were already fixed directly — see below — the rest is backlog):**
  - **The refresh-token flow doesn't actually work.** `auth.js` mints a `RefreshToken` on login/register with no `expiresIn` (so it never expires — a security smell on its own), stores it client-side, but nothing ever redeems it: `APIs/token.js`'s `'auth'` branch calls `generateAccessToken(req.user, ...)` with no `passport.authenticate` middleware on that route, so `req.user` is always `undefined` — that branch is dead code. There's also no axios response interceptor on the frontend to catch a 401 and attempt a refresh. Net effect: the `AccessToken` expires after 1 hour (`jwtgenerator.js`) and the user just starts silently getting 401s with no recovery path short of logging in again. Needs either a real refresh endpoint (validate the refresh token via middleware, mint a fresh access token) wired to a frontend 401 interceptor, or the refresh token should be removed entirely if it's not going to be used — half-implemented is worse than absent.
  - **No brute-force protection anywhere.** No rate limiting (`index.js` has no `express-rate-limit`/`helmet`) and no lockout after repeated failed logins on `/auth/login`. Fine for dev, not fine to launch with.
  - **No email verification.** `POST /auth/register` auto-logs the user in immediately with no confirmed/unconfirmed gating, and the welcome email is currently commented out (`auth.js`'s `// welcomeEmail(...)` call) despite the template existing in `Emails/`.
  - **`DB_User.Phone` is required + unique but not used for anything** (no OTP/verification) — pure registration friction for a constraint that doesn't pay for itself today. Either build real phone verification or drop the uniqueness/required constraint.
  - **`DB_User.Language` field is unused.** The schema already has a `Language` field (default `"English"`) that looks intended to persist a user's language preference server-side/per-account, but nothing reads or writes it — the language switcher built for the homepage/nav/footer is purely client-side (`localStorage`), so a logged-in user's language doesn't follow them across devices. Decide whether to wire this field up or remove it as dead schema.
