...
- **Language switcher wrapper for the UI (in progress, branch: `language-switcher`).** A reusable language-switcher component/wrapper for `react-frontend`, supporting at least English/Chinese, to back the "default UI language from IP region, overridable via a language switcher" behavior described above. Scoped to `react-i18next` + a nav-bar switcher + translating the homepage hero/nav as the initial proof of the mechanism; true IP-region-based default language is deferred to whenever the network-reachability-based provider routing item above actually builds region detection, rather than duplicating that work here. See the two follow-up items below for what's explicitly out of scope in this first pass.
