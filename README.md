...
- **Each day's `Route` (arrival/departure/accommodation waypoints) is computed but not yet drawn on the Itinerary map.** The map (`components/itinerary/Itinerary.js`) still only plots `Spots`, not the fuller `Route` described above — the route line and distinct arrival/departure/accommodation icons are a follow-up piece, tracked in `CLAUDE.md`'s Pending Tasks.
