...
**Each spot card shows a real photo when one can be found, not a fixed placeholder cycle.** `Node/Services/spotPhotos.js` looks up a live photo URL per spot via Amap's place search API (same integration `amapPlaces.js` uses for accommodation lookups), run in parallel across a trip's spots by `itineraryPlanner.js`'s `assignPhotos()`. The photo is just a URL, shown as-is on the Itinerary page — not downloaded or persisted anywhere beyond the trip document, so it's a live lookup on every generation rather than a cached asset. A spot whose lookup fails or comes back empty (no Amap key configured, no match found) falls back to one of four generic placeholder images, cycled round-robin the same way every spot's `Photo` used to be assigned.
